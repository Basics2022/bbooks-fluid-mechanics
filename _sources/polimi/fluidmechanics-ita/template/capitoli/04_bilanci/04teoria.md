...
volume della quantità di moto per unità di volume $\rho \bm{u}$, dove
$\bm{u}$ è la velocità delle particelle materiali. Per la **prima
...
di volume e $\oint_{S(t)} \bm{t_n}$ la risultante delle forze esterne di
superficie, avendo indicato con $\bm{f}$ il campo di forze per unità di
volume e $\bm{t_n}$ il vettore sforzo agente sulla supreficie esterna
$S(t)$ del volume $V(t)$. Il teorema di Cauchy nella meccanica del
continuo, permette di esprimere il vettore sforzo $\bm{t_n}$ in funzione
del tensore degli sforzi $\mathbb{T}$ e la normale alla superficie
$\bm{\hat{n}}$, come $\bm{t_n} = \bm{\hat{n}} \cdot \mathbb{T}$.
...
volume $\rho \bm{r} \times \bm{u}$, dove $\bm{r}$ è il vettore che
...
sua energia cinetica per unità di volume $\rho |\bm{u}|^2/2$. Combinando
...
avendo indicato con $\bm{q}$ il flusso di calore uscente dal volume
...
dovuto alla velocità relativa $\bm{u} - \bm{v}$ tra le particelle
...
superficie $s(t)$ si muove con velocità $\bm{v}$, risultano
$$\begin{cases}
...
fisso $V_c$ risultano $$\begin{cases}
...
$$\oint_{S} f n_i = \int_V f_{/i} \ ,$$ avendo indicato con $f_{/i}$ la
...
continui, $$\bm{t_n} = \bm{\hat{n}} \cdot \mathbb{T} \quad , \quad 
  t_i = n_j T_{ji} \ ,$$ dove $\bm{t_n}$ è il vettore sforzo,
$\bm{\hat{n}}$ la normale alla superficie e $\mathbb{T}$ il tensore
...
indiciale, $$\oint_S \bm{t_n} = \oint_S t_i = \oint_S n_j T_{ji} =
  \int_V T_{ji/j} = \int_V \bm{\nabla} \cdot \mathbb{T} \ .$$ Usando il
...
Sviluppando i termini $$\begin{aligned}
 ...
 ( \rho u_i u_j )_{/j} = \rho u_j u_{i/j} + u_i (\rho u_j)_{/j} 
  \ ,
\end{aligned}$$ riconoscendo che
$\bm{u} \cdot (\partial \rho/\partial t + \bm{\nabla} \cdot (\rho \bm{u}))=0$
...
\end{aligned}$$ avendo utilizzato la simmetria del tensore degli sforzi,
...
gradiente di velocità $\bm{\nabla} \bm{u}$, viene definito **fluido
...
dallo stato termodinamico del sistema. Il flusso di calore $\bm{q}$ per
...
termodinamico del sistema, $$\bm{q} = - k \bm{\nabla} T \ .$$
...
fornisca le relazioni necessarie, $$\begin{aligned}
  p = p(\rho,e) \quad , & \quad \mu = \mu(\rho,e) \\
  T = T(\rho,e) \quad , & \quad \lambda = \lambda(\rho,e) \\
  & \quad k = k(\rho,e) \ ,
 \end{aligned}$$ avendo scelto le variabili termodinamiche delle quali è
...
$\rho|\bm{u}|^2/2$, l'energia interna $e$, e la vorticità
$\bm{\omega} = \nabla \times \bm{u}$.

#### Equazione dell'energia cinetica

Moltiplicando scalarmente il bilancio della quantità di moto per il
vettore velocità $\bm{u}$, si può scrivere l'equazione di bilancio
...
$\mathbb{S} = 2\mu\mathbb{D} + \lambda\bm{\nabla} \cdot \bm{u} \mathbb{I}$,
...
l'entropia, poiché $$T, \, \mu, \, \lambda, \, k \, \geq 0
 \qquad \text{e} \qquad
...
dell'entropia in Termodinamica, $$dS = \dfrac{\delta Q}{T} \ .$$ Questi
due termini possono dare un contributo positivo o nevativo, a seconda
del segno della sorgente di calore $r$ e del flusso di valore $\bm{q}$.
...
entropia $\bm{q}/T$ attraverso il contorno del volume $V(t)$ svolto da
un flusso di calore $\bm{q}$.
