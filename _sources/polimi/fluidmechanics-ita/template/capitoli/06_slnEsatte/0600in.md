...
$\partial \bm{u}/\partial t = \bm{0}$. Un regime di moto instazionario
...
Navier-Stokes per un fluido incomprimibile $$\begin{cases}
 \rho \dfrac{\partial \bm{u}}{\partial t}
 + \rho (\bm{u} \cdot \bm{\nabla}) \bm{u}
 - \mu \nabla^2 \bm{u} + \bm{\nabla} p = \rho \bm{g} \\
 \bm{\nabla} \cdot \bm{u} = 0
\end{cases}$$ accompagnate dalle condizioni iniziali e dalle condizioni
...
di coordinate cartesiane $$\begin{cases}
...
\end{cases}$$ o in un sistema di coordinate cilindriche $$\begin{cases}
...
-   sfruttando la bidimensionalità della corrente, l'omogeneità della
    ...
    parete $\bm{u} = (u,v) = \bm{0}$ è evidente che la costante deve
    essere nulla: $v = 0$.

-   supponiamo qui che, se vengono considerate le forze di volume, esse
    sono costanti e dirette lungo $-\bm{\hat{y}}$.

Le equazioni diventano quindi $$\begin{cases}\label{eqn:poiseuille}
- \mu \left( \dfrac{d^2 u}{d y^2} \right)
  + \dfrac{\partial p}{\partial x} = 0 \\
 \dfrac{\partial p}{\partial y} = - \rho g
\end{cases}$$ dove la derivata parziale in $y$ della componente $u$ è
stata sostituita dalla derivata ordinaria, poiché la velocità
$\bm{u}(y)$ (e quindi tutte le sue componenti) dipende solo dalla
coordinata $y$. La seconda equazione integrata dà come risultato ($p$
dipende sia da $x$ sia da $y$): $$p(x,y) = -\rho g y + f(x)$$ Inserita
nella prima: $$\mu \left( \dfrac{d^2 u}{d y^2} \right) =
...
le due equazioni $$\label{eqn:poiseuille2}
\begin{cases}
  \mu \left( \dfrac{d^2 u}{d y^2} \right) = - G_P \\
  \dfrac{\partial p}{\partial x} = -G_P
\end{cases}$$ accompagnate dalle opportune condizioni al contorno.
...
$\bm{t}_{n,s}$ esercitato sul solido, uguale e contrario allo sforzo
esercitato dal solido sul fluido $\bm{t}_{n}$. Il vettore sforzo
$\bm{t}_n$ su una superficie con giacitura definita dal versore normale
$\bm{\hat{n}}$ si può esprimere come il prodotto del versore
$\bm{\hat{n}}$ e il tensore degli sforzi $\mathbb{T}$,
...
$\bm{s_n} = \bm{\hat{n}} \cdot \mathbb{S}$ è il vettore degli sforzi
...
sforzo come prodotto tensoriale tra il versore normale $\bm{\hat{n}}$ e
...
$$\begin{aligned}
 ...
 \end{aligned}$$ Ad esempio, per una corrente in uno spazio
...
matriciale, $$\mathbb{T} =
 ...
 2 \mu \begin{bmatrix}
  ...
  \dfrac{\partial v}{\partial y} & 
 \end{bmatrix}$$ Sfruttando la simmetria del tensore degli sforzi,
...
$\bm{u}(\bm{r}) = u(y) \bm{\hat{x}}$. Facendo riferimento alla corrente
...
$\bm{\hat{n}} = - \bm{\hat{y}}$ per il tensore degli sforzi,
$$\begin{aligned}
...
\end{aligned}$$ Lo sforzo sulla parete inferiore è l'opposto
$\bm{t}_{n,s} = \frac{G_P H}{2} \bm{\hat{x}} - p \bm{\hat{y}}$. Sulla
parete superiore, a $y=H$, la normale uscente dal fluido è
$\bm{\hat{n}} = \bm{\hat{y}}$, la derivata
...
parete superiore è
$\bm{t}_{n,s} = \frac{G_P H}{2} \bm{\hat{x}} + p \bm{\hat{y}}$.
...
$\bm{\hat{n}} \times \bm{\nabla} \times \bm{u}$ è $$\begin{aligned}
...
\end{aligned}$$ Il contributo viscoso al vettore sforzo è uguale al
...
$\bm{\hat{n}} \cdot \mathbb{T}$ svolgendo un prodotto matrice-vettore
...
gradiente $\bm{\nabla} \bm{v}$ di un campo vettoriale $\bm{v}$ sono
...
\end{bmatrix} \ ,$$ se riferite alla base fisica
$(\bm{\hat{r}},\bm{\hat{\theta}},\bm{\hat{z}})$. Bisogna quindi prestare
...
$2 \mu \bm{\hat{n}} \cdot \mathbb{D}$. Poichè il sistema di coordinate
cilindriche (fisiche, riferite alla base
$(\bm{\hat{r}},\bm{\hat{\theta}},\bm{\hat{z}})$) è un sistema
...
\end{bmatrix}
 \begin{bmatrix} n_{r} \\ n_{\theta} \\ n_z \end{bmatrix} \ .$$ Come
